...
class Config(object):
    ...
    # 1.模型保存路径
    save_path = 'imgs/'  # opt.netg_path生成图片的保存路径
    # 判别模型的更新频率要高于生成模型
    d_every = 1  # 每一个batch 训练一次判别器
    g_every = 5  # 每1个batch训练一次生成模型
    save_every = 10  # 每save_every次保存一次模型
    netd_path = "imgs/netd_199.pth"
    netg_path = "imgs/netg_199.pth"
...
def train(**kwargs):
    ...
    for epoch in range(opt.max_epoch):
        ...
        if (epoch + 1) % opt.save_every == 0:
            ...
            torch.save(netd.state_dict(),  'imgs/' + 'netd_{0}.pth'.format(epoch))
            torch.save(netg.state_dict(),  'imgs/' + 'netg_{0}.pth'.format(epoch))
...
def generate(**kwargs):
    ...
    netd.load_state_dict(torch.load('imgs/netg_199.pth', map_location=map_location), False)
    netg.load_state_dict(torch.load('imgs/netg_199.pth', map_location=map_location), False)
    netd.to(device)
    netg.to(device)
    # 生成训练好的图片
