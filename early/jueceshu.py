...
def info_entropy(df, predict_attr):
    ...
    if p == 0 or n == 0:
        ...
        I = ((-1 * p) / (p + n)) * math.log(p / (p + n), 2) + (
                    (-1 * n) / (p + n)) * math.log(n / (p + n), 2)
    return I
...
def remainder(df, df_subsets, predict_attr):
    ...
    for df_sub in df_subsets:
        if df_sub.shape[0] > 1:
            remainder += float(df_sub.shape[0] / num_data) * info_entropy(
                df_sub, predict_attr)
    return remainder
...
def info_gain(df, attribute, predict_attr, threshold):
    ...
    # and subtract remainder of attributes from it
...
def test_predictions(root, df):
    num_data = df.shape[0]
    num_correct = 0
    print(f'Index\t(Outcome, Prediction)')
    for index, row in df.iterrows():
        prediction = predict(root, row)
        print(f'{index}\t({row["Outcome"]}, {prediction})')
...
def clean(csv_file_name):
    ...
    df.columns = ['K', 'Na', 'CL', 'HCO', 'Endotoxin', 'Anioingap', 'PLA2', 'SDH', 'GLDH', 'TPP', 'Breath rate', 'PCV', 'Pulse rate',
                'Fibrinogen', 'Dimer', 'FibPerDim', 'Diagnosis']
    df['Outcome'] = 0
...
def main():
    ...
    print("\n",
          ...
          str(test_predictions(root, df_test) * 100.0) + '%')
